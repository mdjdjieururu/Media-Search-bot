...
async def cb_data(bot, update):
	    if update.data == "home":
            await query.answer("സിനിമ ലഭിക്കണം എങ്കിൽ താങ്കൾ ɢᴏᴏɢʟᴇ ɪᴍᴅʙ താഴെ ʟɪɴᴋ കൊടുക്കാo പോയി നോക്ക് സിനിമയുടെ ᴄᴏʀʀᴇᴄᴛ ꜱᴘᴇʟʟɪɴɢ ഇവിടെ ꜱᴇɴᴅ ചെയ്യുക എങ്കിലേ താങ്കൾ ഉദ്ദേശിക്കുന്ന സിനിമ എനിക്ക് അയച്ചു തരാൻ കഴിയുകയുള്ളു\n\nʙʏ: ᴍᴏᴠɪᴇ ꜰᴀᴄᴛᴏഞാൻ", show_alert=True)
        ...
        buttons.append(
            [InlineKeyboardButton(text=f"◉ ᴘᴀɢᴇꜱ ɴᴜᴍʙᴇʀꜱ ◉ ❮1-{data['total']}❯",callback_data="pages")]
        )
        poster=None
        if API_KEY:
            poster=await get_poster(search)
